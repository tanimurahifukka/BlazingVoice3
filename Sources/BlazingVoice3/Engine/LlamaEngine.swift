...
final class LlamaEngine: InferenceEngine, @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "blazing.llama.engine")
    ...
    private var activeJobs: [Int: GenerationJob] = [:]
    ...
    init(modelPath: String, slotCount: Int = 4, perSlotCtx: UInt32 = 4096) throws {
        ...
        guard let model = llama_model_load_from_file(modelPath, modelParams) else {
            throw BlazingError.modelLoadFailed(modelPath)
        }
        self.model = model

        guard let vocab = llama_model_get_vocab(model) else {
            throw BlazingError.vocabLoadFailed
        }
        self.vocab = vocab
        ...
        guard let context = llama_init_from_model(model, ctxParams) else {
            throw BlazingError.contextCreationFailed
        }
    ...
    static func withQuantization(
        ...
        if kvQuantize != .off {
            print("[KVCache] Quantization: \(kvQuantize.displayName)")
        }
        ...
        KVCacheQuantizerConfig.shared.mode = .off
        return engine
    }
    ...
    func generate(
        ...
        guard !tokens.isEmpty else {
            throw BlazingError.tokenizationFailed
        }
        if tokens.count + maxTokens > Int(perSlotCtx) {
            throw BlazingError.contextExceeded
        }
    ...
    private func schedulerTick() {
        ...
        if sharedBatch.n_tokens > 0 {
            ...
            if rc != 0 {
                for (_, job) in logitsMap {
                    job.fail(error: BlazingError.decodeFailed)
                }
            } else {
        ...
        if !pendingJobs.isEmpty || !activeJobs.isEmpty {
            queue.async { [self] in schedulerTick() }
        } else {
    ...
    private func admitPendingJobs() {
        ...
        for (i, job) in pendingJobs.enumerated() {
            ...
            guard let sampler = llama_sampler_chain_init(sparams) else {
                job.fail(error: BlazingError.samplerCreationFailed)
